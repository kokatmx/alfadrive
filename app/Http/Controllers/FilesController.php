...
class FilesController extends Controller
{
    ...
    // public function index()
    // {
    //     $folderId = $this->getFolderId(); // Ambil Folder ID dari konfigurasi
    //     $apiKey = config('services.google.api_key'); // Pastikan Anda menyimpan API Key di .env dan config
    ...
    //             return view('google_drive', ['files' => $files['files']]);
    //         } else {
    //             return response()->json(['message' => 'Tidak ada file ditemukan']);
    //         }
    //     } catch (\Exception $e) {
    //         // Penanganan kesalahan
    //         return response()->json(['error' => $e->getMessage()], 500); // Kembalikan pesan kesalahan
